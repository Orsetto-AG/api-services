...
export class GCPService {
    ...
    public async copyFile(srcFileName: string, destFileName: string): Promise<any> {
        const filePath = `orsetto/${srcFileName}`;
        const destFilePath = `orsetto/${destFileName}`;
    ...
    public deleteFile(fileName: string = ''): Promise<any> {
        return bucket.file(`orsetto/${fileName}`).delete();
    }

    public deleteFiles(prefix: string = ''): Promise<void> {
        return new Promise((resolve, reject) => {
            return bucket.deleteFiles({ prefix: `orsetto/${prefix}` }, err => {
                if (err) {
                    return reject(err);
                }
                return resolve();
